...
| 8 | Environment variable injection (LD_PRELOAD) | **Partially true, MITIGATED in PR #12** | Gateway validates `params.env` via blocklist (`src/agents/bash-tools.exec-runtime.ts:32-50` and `src/infra/exec-approvals-allowlist.ts:41-200` — was `src/agents/bash-tools.exec.ts:61-78,976-977`). Node-host has blocklist (`src/node-host/invoke.ts:44-174` — was `src/node-host/runner.ts:166-175`). Requires human approval + localhost + no sandbox. |
