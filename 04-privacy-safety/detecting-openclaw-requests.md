...
**Source:** `src/agents/tools/web-search.ts:448-449`
```typescript
"HTTP-Referer": "https://openclaw.ai",
"X-Title": "OpenClaw Web Search",
```
...
**Source:** `src/agents/tools/web-search.ts:502-508`
```typescript
headers: {
  "Content-Type": "application/json",
...
| `X-Title: OpenClaw Web Search` | `src/agents/tools/web-search.ts:449` | Perplexity search |
...
| Trusted proxies | `gateway.trustedProxies` | Cloudflare IP ranges | Gateway trusts `X-Forwarded-For` / `X-Real-IP` from these IPs for client IP resolution (`src/gateway/net.ts:106-120`) |
