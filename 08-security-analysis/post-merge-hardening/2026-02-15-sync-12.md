...
| **LOW** | `28b78b25b` | **Persist bootstrap onboarding state:** New `workspace-state.json` tracking via `resolveWorkspaceStatePath()` at `src/agents/workspace.ts:172`, `readWorkspaceOnboardingState()` at `:197-215`, `writeWorkspaceOnboardingState()` at `:228-242`. Records which bootstrap files have been acknowledged. Causes +122 line shift (now further shifted by Feb 20 sync 5 f1e1cc4ee +29): `loadWorkspaceBootstrapFiles()` now at `441-495`, `filterBootstrapFilesForSession()` now at `499-507`. |
