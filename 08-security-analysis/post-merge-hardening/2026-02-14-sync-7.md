...
| `validateHostEnv` | `bash-tools.exec.ts:83-107` | `bash-tools.exec-runtime.ts:34` (def) + `bash-tools.exec.ts:330` (call) | Was line 54/295 after PR #12; shifted to 34/330 in Feb 21 sync 7 (`f202e7307`) |
...
| `blockedEnvKeys` | `runner.ts:166-175` | `host-env-security-policy.json` (moved to JSON in Feb 21 sync 7); `sanitizeEnv()` at `invoke.ts:115` delegates to `sanitizeHostExecEnv()` at `host-env-security.ts:46` | PR #12 placed inline blocklist in invoke.ts; moved to centralized policy by Feb 21 sync 7 (`f202e7307`) |
