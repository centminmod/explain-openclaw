...
- **`01b3226ec`** — **fix(gateway): block node.invoke exec approvals:** Blocks `system.execApprovals.get` and `system.execApprovals.set` from the `node.invoke` pathway (`src/gateway/server-methods/nodes.ts:371-381`). Returns explicit error: `"node.invoke does not allow system.execApprovals.*; use exec.approvals.node.*"`. **Addresses Audit 2 Claim 5** (self-approving agent) — prevents nodes from manipulating exec approval state through the invoke pathway.
