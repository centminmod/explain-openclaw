...
- `src/config/zod-schema.providers-core.ts:841` — `remoteHost` field uses `.refine(isSafeScpRemoteHost, ...)`
...
- `src/config/zod-schema.session.ts:161` — `restart: z.boolean().optional().default(true)`
