...
3. **`e97aa4542`** — **fix(windows): handle undefined environment variables in runCommandWithTimeout:** `src/process/exec.ts:119-120` now filters `undefined` values from environment entries and coerces all values to strings via `String(value)`. Prevents Windows `spawn()` from receiving literal `undefined` as environment variable value, which could cause unpredictable behavior.
